...
def train(args):
    ...
    scheduler = lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.97, verbose=True, patience=2)
    #lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer,step_size=10,gamma=0.1)
    #lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma = 0.95, last_epoch=-1)
    #lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[23,37], gamma=0.1)
    ...
    for epoch in range(args.n_epoch):
        ...
        #    warmup_iters = len(trainloader) - 1

        #    lr_scheduler_iter = utils.warmup_lr_scheduler(optimizer, warmup_iters, warmup_factor)
        ...
        for i, (images, labels) in enumerate(trainloader):
            ...
            #    writer.add_scalar('train/iteration_lr', optimizer.param_groups[0]["lr"], i+1)
            #    lr_scheduler_iter.step()
