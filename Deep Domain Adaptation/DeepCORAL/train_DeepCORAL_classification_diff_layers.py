...
from core.models import model_classification_diff_layers as DeepCORAL_model
import core.loss
...
#<--------------------------------------------------------------------------NOT NECESSARY-------------------------------------------------------------------------->
    ...
    if args.resume is not None:
        if os.path.isfile(args.resume):
            ...
            torch.nn.init.normal_(model.module.shared.fc_last.weight, mean=0, std=5e-3)
            model.module.shared.fc_last.bias.data.fill_(0.01)
            ...
            if hasattr(model.module, 'optimizer'):
                ...
                if args.optim in ["sgd", "SGD", "Sgd"]:
                    optimizer = torch.optim.SGD([{'params': model.module.shared.start.parameters()}, {'params': model.module.shared.encode1.parameters()},
                                                 {'params': model.module.shared.encode2.parameters()}, {'params': model.module.shared.encode3.parameters()},
                                                 {'params': model.module.shared.encode4.parameters()}, {'params': model.module.shared.encode5.parameters()},
                                                 {'params': model.module.shared.middle.parameters()}, {'params': model.module.shared.fc1.parameters()},
                                                 {'params': model.module.shared.fc2.parameters()}, {'params': model.module.shared.fc3.parameters()},
                                                 {'params': model.module.shared.fc_last.parameters(), 'lr': 10 * args.base_lr}
                                                ],lr=args.base_lr, weight_decay=0.0001, momentum=0.9)
                elif args.optim in ["adam", "ADAM", "Adam"]:
                    optimizer = torch.optim.Adam([{'params': model.module.shared.start.parameters()}, {'params': model.module.shared.encode1.parameters()},
                                                 {'params': model.module.shared.encode2.parameters()}, {'params': model.module.shared.encode3.parameters()},
                                                 {'params': model.module.shared.encode4.parameters()}, {'params': model.module.shared.encode5.parameters()},
                                                 {'params': model.module.shared.middle.parameters()}, {'params': model.module.shared.fc1.parameters()},
                                                 {'params': model.module.shared.fc2.parameters()}, {'params': model.module.shared.fc3.parameters()},
                                                 {'params': model.module.shared.fc_last.parameters(), 'lr': 10 * args.base_lr}
                                                ],lr=args.base_lr, weight_decay=0.0001, amsgrad=True)
                elif args.optim in ["adadelta", "ADADELTA", "AdaDelta", "Adadelta"]:
                    optimizer = torch.optim.Adadelta([{'params': model.module.shared.start.parameters()}, {'params': model.module.shared.encode1.parameters()},
                                                 {'params': model.module.shared.encode2.parameters()}, {'params': model.module.shared.encode3.parameters()},
                                                 {'params': model.module.shared.encode4.parameters()}, {'params': model.module.shared.encode5.parameters()},
                                                 {'params': model.module.shared.middle.parameters()}, {'params': model.module.shared.fc1.parameters()},
                                                 {'params': model.module.shared.fc2.parameters()}, {'params': model.module.shared.fc3.parameters()},
                                                 {'params': model.module.shared.fc_last.parameters(), 'lr': 10 * args.base_lr}
                                                ], lr=args.base_lr, rho=0.9, eps=1e-06, weight_decay=0.0001)
                else:
    ...
    for epoch in range(args.n_epoch):
        ...
        for i, ((source_image, source_label), (target_image, _)) in enumerate(zip(source_trainloader, target_trainloader)):
            ...
            if args.backbone in ["DeepLab_ResNet9", "deeplab_resnet9", "deeplab", "resnet9", "ResNet9", "DeepLab"]:
                fc_last_source, fc3_source, fc2_source, fc1_source , fc_last_target, fc3_target, fc2_target, fc1_target = model(source_image, target_image)
            elif args.backbone in ["SeismicNet", "seismicnet"]:
                fc_last_source, fc3_source, fc2_source, fc1_source , fc_last_target, fc3_target, fc2_target, fc1_target = model(source_image, target_image)
            else:
                print("Unknown Backbone, available options:[DeepLab_ResNet9, SeismicNet]")
            
            pred = fc_last_source.detach().max(1)[1].cpu().numpy()
            gt = source_label.detach().cpu().numpy()
            
            running_metrics.update(gt, pred)

            classification_loss = loss_fn(input=fc_last_source, target=torch.squeeze(source_label), weight=class_weights)
            ...
            if args.backbone in ["DeepLab_ResNet9", "deeplab_resnet9", "deeplab", "resnet9", "ResNet9", "DeepLab"]:
                ...
                coral_loss3 = DeepCORAL_model.CORAL(fc3_source, fc3_target)
                coral_loss2 = DeepCORAL_model.CORAL(fc2_source, fc2_target)
                coral_loss1 = DeepCORAL_model.CORAL(fc1_source, fc1_target)
                coral_loss = coral_loss3 + coral_loss2 + coral_loss1
            else:
                print("Unknown Backbone, available options:[DeepLab_ResNet9, SeismicNet]")
            ...
            coral_loss_weight = 0
            coral_loss = coral_loss * _lambda
            sum_loss = coral_loss + classification_loss
            ...
            with torch.no_grad():  # operations inside don't track history
                ...
                for i_val_source, (source_image_val, source_label_val) in enumerate(source_valloader):
                    ...
                    if args.backbone in ["DeepLab_ResNet9", "deeplab_resnet9", "deeplab", "resnet9", "ResNet9", "DeepLab"]:
                        ...
                        source_outputs_val, _, _, _, _, _, _, _ = model(source_image_val, source_image_val)
                    else:
                        print("Unknown Backbone, available options:[DeepLab_ResNet9, SeismicNet]")
                ...
                for i_val_target, (target_image_val, target_label_val) in enumerate(target_valloader):
                    ...
                    if args.backbone in ["DeepLab_ResNet9", "deeplab_resnet9", "deeplab", "resnet9", "ResNet9", "DeepLab"]:
                        ...
                        target_outputs_val, _, _, _, _, _, _, _ = model(target_image_val, target_image_val)
                    else:
                        print("Unknown Backbone, available options:[DeepLab_ResNet9, SeismicNet]")
