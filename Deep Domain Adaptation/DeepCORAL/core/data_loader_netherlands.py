...
class PatchLoader(data.Dataset):
    ...
    def __init__(self, split='train', is_transform=True, augmentations=None):
        ...
        if 'train' in self.split: 
            ...
            self.seismic = np.load(pjoin('data_netherlands', 'train', 'images.npy'))
            self.labels = np.load(pjoin('data_netherlands', 'train', 'labels.npy'))
            self.mean = np.round(np.mean(self.seismic), 6) # average of the training data
        elif 'valid' in self.split:
            self.seismic = np.load(pjoin('data_netherlands', 'valid', 'images.npy'))
            self.labels = np.load(pjoin('data_netherlands', 'valid', 'labels.npy'))
            self.mean = np.round(np.mean(self.seismic), 6) # average of the training data
        else:
