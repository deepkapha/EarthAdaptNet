...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.downsample(x)
        return self.act(x + residual), x + residual
...
class encoder(nn.Module):
    def __init__(self, in_channels=1):
        ...
        self.middle = nn.Sequential(nn.Conv2d(1024, 1024, 1))
    ...
    def forward(self, x):
        ...
        # Encoder
        x1, _ = self.encode1(x)
        x2, _ = self.encode2(x1)
        x3, feature_map_3 = self.encode3(x2)
        x4, feature_map_4 = self.encode4(x3)
        x5, feature_map_5 = self.encode5(x4)

        out_middle = self.middle(x5)

        #return out_middle, x, x1, x2, x3, x4, feature_map_3, feature_map_4, feature_map_5
        return out_middle, x, x1, x2, x3, x4
...
class decoder(nn.Module):
    ...
    def forward(self, out_middle, x, x1, x2, x3, x4):
        
        out_middle = self.middle(out_middle)
        # Decoder
...
def seismicnet_encoder():
    
    model_encoder = encoder()
    
    return model_encoder
    
def seismicnet_decoder(n_classes):
    
    model_decoder = decoder(n_classes)
